...
async def run_test():
    ...
    try:
        ...
        # Try to scroll down or reload page to reveal login form
        await page.mouse.wheel(0, window.innerHeight)
        

        # Navigate to 'Usuarios' (Users) module to create a new user with limited permissions.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/div/nav/a[15]').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Click on 'Nuevo Usuario' button to start creating a new user.
        frame = context.pages[-1]
        elem = frame.locator('xpath=html/body/div/div/div[2]/div[2]/main/div/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # Fill in the new user details with a username, email, name, surname, uncheck 'Es administrador', ensure 'Usuario activo' is checked, then create the user.
        frame = context.pages[-1]
