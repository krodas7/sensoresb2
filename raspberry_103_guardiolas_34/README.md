# 🌡️ Raspberry Pi 192.168.0.103 - Guardiolas 3 y 4
...
```bash
# Crear directorio
mkdir -p /home/pi/api-beneficio/raspberry_103_guardiolas_34

# Copiar archivos desde tu computadora:
# - raspberry_temp_client.py
# - requirements.txt
# - install_guardiolas_34_service.sh
# - guardiolas-34-sensor-client.service
```

### Paso 4: Instalar Dependencias y Servicio
```bash
cd /home/pi/api-beneficio/raspberry_103_guardiolas_34
...
sudo systemctl start guardiolas-34-sensor-client
```
...
# Detener servicio
sudo systemctl stop guardiolas-34-sensor-client

# Reiniciar servicio
sudo systemctl restart guardiolas-34-sensor-client

# Ver estado
sudo systemctl status guardiolas-34-sensor-client

# Habilitar inicio automático
sudo systemctl enable guardiolas-34-sensor-client
...
tail -f /home/pi/api-beneficio/raspberry_103_guardiolas_34/guardiolas_34_temp_client.log
...
GPIO.setup(8, GPIO.OUT)  # CS Guardiola 3
...
# - CS conectado a GPIO8 (Guardiola 3) o GPIO7 (Guardiola 4)
...
- **Logs**: Registrados en `/var/log/guardiolas-34-restart.log`
- **Verificación**: Automática después de cada reinicio

### Comandos de Gestión del Reinicio
```bash
# Ver estado del timer de reinicio
sudo systemctl status guardiolas-34-restart.timer

# Ver logs de reinicios programados
sudo journalctl -u guardiolas-34-restart.service

# Forzar reinicio programado ahora
sudo systemctl start guardiolas-34-restart.service

# Ver cuándo será el próximo reinicio
sudo systemctl list-timers guardiolas-34-restart.timer

# Deshabilitar reinicio automático
sudo systemctl disable guardiolas-34-restart.timer
...
- **Servicio Principal**: `/etc/systemd/system/guardiolas-34-sensor-client.service`
- **Servicio de Reinicio**: `/etc/systemd/system/guardiolas-34-restart.service`
- **Timer de Reinicio**: `/etc/systemd/system/guardiolas-34-restart.timer`
- **Logs del Servicio**: `/var/log/guardiolas-34/guardiolas_34_temp_client.log`
- **Logs de Reinicio**: `/var/log/guardiolas-34-restart.log`
- **Código**: `/home/laptop/guardiolas_34/raspberry_temp_client.py`
...
## 🔗 Diferencias con Raspberry Pi 102

Esta Raspberry Pi es **idéntica** a la 192.168.0.102 en configuración:
- **Mismos pines**: GPIO8 y GPIO7 para CS
- **Mismo protocolo**: SPI con MAX6675
- **Mismo intervalo**: 30 segundos
- **Misma API**: Mismos endpoints

La única diferencia es:
- **IP**: 192.168.0.103 vs 192.168.0.102
- **Nombres**: "Guardiola 3" y "Guardiola 4" vs "Guardiola 1" y "Guardiola 2"

---

**¡Sistema listo para monitorear la temperatura de las Guardiolas 3 y 4!** 🌡️
