...
# Script para gestionar el reinicio programado del servicio de fermentación
# API Beneficio - Sistema de monitoreo automático

SERVICE_NAME="fermentacion-sensor-client"
TIMER_NAME="fermentacion-restart.timer"
RESTART_SERVICE_NAME="fermentacion-restart.service"
...
show_help() {
    echo -e "${BLUE}=== Gestor de Reinicio Programado - API Beneficio ===${NC}"
    echo ""
    echo "Uso: $0 [comando]"
    echo ""
...
show_status() {
    echo -e "${BLUE}=== Estado del Sistema de Fermentación ===${NC}"
    echo ""
