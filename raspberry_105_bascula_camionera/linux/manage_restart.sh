...
# Script para gestionar el reinicio programado del servicio de báscula camionera
# API Beneficio - Sistema de monitoreo automático

SERVICE_NAME="bascula-camionera-sensor-client"
TIMER_NAME="bascula-camionera-restart.timer"
RESTART_SERVICE_NAME="bascula-camionera-restart.service"
...
show_help() {
    echo -e "${BLUE}=== Gestor de Reinicio Programado - API Beneficio (Báscula Camionera) ===${NC}"
    echo ""
    echo "Uso: $0 [comando]"
    echo ""
...
show_status() {
    echo -e "${BLUE}=== Estado del Sistema de Báscula Camionera ===${NC}"
    echo ""
